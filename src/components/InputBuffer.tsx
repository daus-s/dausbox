...
import { INDENT, PROMPT, contPrompt, depthOf, expandForEdit, recallCaret } from "./promptFormat";
...
export default function InputBuffer({
  ...
  }, [buffer, caretIdx, cmdIdx, draft, dausbox, onExec, scrollToBottom, depth, lockedLines]);
    ...
    <>
      {lockedLines.map((line, i) => (
        <p className="buffer" key={i}>
          <span className="prompt">{i === 0 ? PROMPT : contPrompt(depthOf(line))}</span>
          {line.trimStart()}
        </p>
      ))}
      <p className="buffer">
        <span className="prompt">{lockedLines.length === 0 ? PROMPT : contPrompt(depth)}</span>
        {before}
